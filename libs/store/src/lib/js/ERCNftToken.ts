...
import { ERCNftBalance, ERCNftTokenInput } from '../types/ercNft.types';
...
const ERC721_TRANSFER_INPUT =
  IERC721Abi.find((e) => e.name === 'Transfer')?.inputs ?? [];
...
class ERCNftToken {
  ...
  async getAllTokenData(address: string): Promise<string[]> {
    ...
    if (this.data.type === 'ERC1155') {
      for (const tid of ids)
        res.push(await this.getTokenURI(parseInt(tid.tokenId)));
    } else {
      for (const tid of ids)
        res.push(await this.getTokenURI(parseInt(tid.tokenId)));
    }
  ...
  async getTokenURI(id: number) {
    ...
    if (data.startsWith('ipfs://'))
      data = 'https://ipfs.io/ipfs' + data.substring(7);
