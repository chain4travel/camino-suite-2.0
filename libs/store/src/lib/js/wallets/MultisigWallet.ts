...
class MultisigWallet extends WalletCore implements AvaWalletCore {
  ...
  constructor(alias?: Buffer, memo?: string, owner?: Owner) {
    ...
    this.keyData = {
      ...
      memo: parsedMemo,
      owner:
        owner ?? ({ addresses: [], threshold: 0, locktime: ZeroBN } as Owner),
    };
  ...
  getUnlinkedOwners(): string[] {
    return this.unlinkedOwners.map((o) =>
      bintools.addressToString(this.hrp, 'X', o)
    );
  }
  ...
  async updateUTXOsX(): Promise<AVMUTXOSet> {
    const response = await ava
      .XChain()
      .getUTXOs([this._aliasAddress(this.chainId)]);
  ...
  async updateUTXOsP(): Promise<PlatformUTXOSet> {
    const response = await ava
      .PChain()
      .getUTXOs([this._aliasAddress(this.pchainId)]);
    ...
    this.wallets.forEach((w) => {
      ...
      if (key) {
        ...
        if (!kcData.signers.has(address.toString('hex'))) {
          signerFound = true;
          kcData.kc.addKey(
            new MultisigKeyPair(kcData.kc, address, emptyBuffer)
          );
        }
      }
    });
