...
class LedgerWallet extends HdWalletCore implements AvaWalletCore {
  ...
  >(
    ...
    if (destinationChain === 'C') {
      ...
      let changeIdx =
        changePath?.toPathArray()[changePath?.toPathArray().length - 1];
  ...
  >(unsignedTx: UnsignedTx, chainId: ChainIdType): ILedgerBlockMessage[] {
    ...
    if (
      ...
      messages.push({
        title: 'Total Stake',
        value: `${stakeAmt} ${this.nativeAssetSymbol}`,
      });
      messages.push({
        title: 'Stake',
  ...
  >(unsignedTx: UnsignedTx, chainId: ChainIdType): ILedgerBlockMessage[] {
    ...
    if (
      ...
      messages.push({
        title: 'Fee',
        value: `${0.001} ${this.nativeAssetSymbol}`,
      });
    }
  ...
  async signX(unsignedTx: AVMUnsignedTx): Promise<AVMTx> {
    ...
    let { paths, isAvaxOnly } = this.getTransactionPaths<AVMUnsignedTx>(
      unsignedTx,
      chainId
    );
  ...
  async signP(
    ...
    if (txType === PlatformVMConstants.IMPORTTX) {
      ...
      // If destination chain is C chain, sign hash
      const sourceChain = aliasFromChainId(
  ...
  async mintNft(
    ...
    return await WalletHelper.mintNft(
      this,
      mintUtxo,
      payload,
      quantity,
