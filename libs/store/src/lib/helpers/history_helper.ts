  ...
  for (var i = 0; i < nfts.length; i++) {
    ...
    if (intersect.length > 0) {
      ...
      for (var n = 0; i < nftsOuts.length; n++) {
        ...
        let addrNotAdded = nftOut.addresses.filter(
...
function getGainNFT(
  ...
  let walletAddrs = wallet.getHistoryAddresses();
  let addrsStripped = new Set<string>(
    walletAddrs.map((addr) => addr.split('-')[1])
  );
  ...
  for (var i = 0; i < nftsOuts.length; i++) {
    ...
    if (intersect.length > 0) {
      ...
      for (var n = 0; n < nftsIns.length; n++) {
        ...
        let addrNotAdded = nftIn.addresses.filter(
...
function getRawLoss(
  ...
  if (ins) {
    for (const input of ins) {
      ...
      const depositBN = IsRawOutputDeposited(utxo.getOutput())
        ? amountBN
        : ZeroBN;
...
function getRawProfit(
  ...
  if (outs) {
    for (const out of outs) {
      ...
      const depositBN = IsRawOutputDeposited(out.getOutput())
        ? amountBN
        : ZeroBN;
...
function strippedAddresses(addrs: Buffer[]): string[] {
  return addrs.map((addr) =>
    bintools.addressToString(ava.getHRP(), 'P', addr).slice(2)
  );
}
